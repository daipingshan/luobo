...
class CommonController extends CommonBaseController {
    ...
    public function __construct() {
        ...
        if ($this->checkUser) {
            $this->_checkUser();
            if (C('AUTH_ON') == true && !in_array($this->user_id, C('AUTH_ID'))) {
    ...
    protected function _getRuleId($user_id) {
        ...
        if ($auth_group_access) {
            ...
            if (count($rules) > 1) {
                foreach ($rules as $v) {
                    $rule_id = array_merge($rule_id, explode(',', $v));
                }
    ...
    protected function _setMenu($rule_id, $user_id) {
        if ($rule_id) {
            ...
            foreach ($parent_list as $val) {
                $menu_data[$val['id']] = array('title' => $val['title'], 'icon' => $val['icon'], 'href' => $val['name']);
            }
            foreach ($list as $val) {
                if (isset($menu_data[$val['parent_id']]) && isset($menu_data[$val['parent_id']])) {
                    $menu_data[$val['parent_id']]['children'][] = array('title' => $val['title'], 'icon' => $val['icon'], 'href' => $val['name']);
                }
            }
        }
        if (empty($menu_data)) {
            $list      = M('auth_rule')->find(1);
            $menu_data = array(array('title' => $list['title'], 'icon' => $list['icon'], 'href' => $list['name']));
        }
...
/**
     * 获取管理员缓存
     */
    protected function _getAdminCache() {
        $data = S('admin');
        if (empty($data)) {
