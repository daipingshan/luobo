...
class WxPayDataBase
{
	...
	* @param string $value 
	**/
	public function SetSign($md5_key='')
	{
		$sign = $this->MakeSign($md5_key);
	...
	**/
	 ...
	 */
	public function MakeSign($md5_key='')
	{
        if(!trim($md5_key)){
            $md5_key = WxPayConfig::KEY;
        }
		//签名步骤一：按字典序排序参数
		ksort($this->values);
		$string = $this->ToUrlParams();
		//签名步骤二：在string后加入KEY
		$string = $string . "&key=".$md5_key;
		//签名步骤三：MD5加密
		$string = md5($string);
		//签名步骤四：所有字符转为大写
...
{	
	...
	**/
	...
	{
		$this->values['total_fee'] = (int)$value;
	}
	/**
	* 设置订单总金额，只能为整数，详见支付金额
	* @param string $value 
	**/
	public function SetTotal_fee_new($value)
	{
