        ...
        <div class="layui-row">
            <div class="layui-col-xs6 layui-col-sm6 layui-col-md3">
                <select name="project_type_id">
                    ...
                    <?php if(is_array($project_type_data)): foreach($project_type_data as $key=>$val): ?><option value="<?php echo ($val["id"]); ?>"
                        <?php if(I('get.project_type_id') == $val['id']): ?>selected<?php endif; ?>
                        ><?php echo ($val["name"]); ?></option><?php endforeach; endif; ?>
                </select>
            </div>
            <div class="layui-col-xs6 layui-col-sm6 layui-col-md3">
                <input type="text" name="mobile" value="<?php echo I('get.mobile');?>" placeholder="请输入联系电话" class="layui-input">
            </div>
            <div class="layui-col-xs6 layui-col-sm6 layui-col-md3">
                <input type="text" name="name" value="<?php echo I('get.name');?>" placeholder="请输入联系人" class="layui-input">
            </div>
            <div class="layui-col-xs6 layui-col-sm6 layui-col-md3">
...
<script>
    ...
    <script type="text/html" id="table-edit">
        <a class="layui-btn layui-btn-xs layui-btn-warm"
           href="<?php echo U('update');?>?id={{d.id}}">编辑</a>
        <a class="layui-btn layui-btn-xs set-status layui-btn-normal" data-val="{{d.status}}"
           data-url="<?php echo U('setProjectStatus');?>" data-id="{{d.id}}">设置招聘状态</a>
        {{#  if(d.is_recommend == 0){ }}
        <a class="layui-btn layui-btn-xs confirm" data-url="<?php echo U('setProjectRecommend');?>?id={{d.id}}&is_recommend=1">推荐到首页</a>
        {{#  } else { }}
        <a class="layui-btn layui-btn-xs layui-btn-danger confirm"
           data-url="<?php echo U('setProjectRecommend');?>?id={{d.id}}&is_recommend=0">取消推荐</a>
        {{#  } }}
    ...
    <script>
        $(function () {
            ...
            form.on("submit(save-status)", function (data) {
                var _btn = data.elem;
                _btn.disabled = true;
                var url = "<?php echo U('setProjectStatus');?>";
